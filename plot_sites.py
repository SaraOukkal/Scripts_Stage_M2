...
def ancestral_sub_sites(input_AB, output_dir): #plot du nombre de sites par distace aux barrières par type de base
	...
	plt.yticks(fontsize=15)
	plt.axvline(0, color='black', alpha=0.5)
	plt.title("Number of poly nt around NIEBs per nt types", fontsize=20)
	plt.xlabel("Distance from NIEBs", fontsize=16)
	plt.ylabel("Number of A/C/G/T", fontsize=16)
	filename= "ACGT_poly.png"
	filepath=os.path.join(output_dir, filename)
	plt.savefig(filepath)
	plt.clf()	
...
def ancestral_sub_sites_percent(bar_AB_count, output_dir): #plot du pourcentage de bases par distace aux barrières
	...
	plt.yticks(fontsize=20)
	plt.axvline(0, color='black', alpha=0.5)
	plt.title("Percentages of poly nt around NIEBs", fontsize=20)
	plt.xlabel("Distance from NIEBs", fontsize=16)
	plt.ylabel("Percentages of A/C/G/T", fontsize=16)
	filename= "ACGT_poly_per.png"
	filepath=os.path.join(output_dir, filename)
	plt.savefig(filepath)
	plt.clf()
