...
def GC_content(input_AB1, input_Gen1,input_AB2, input_Gen2, output_dir):
	...
	plt.plot(dist,GC_AB, color='lightcoral')
	plt.plot(dist,GC_Gen, color='lightcoral', linestyle='dashed')
	plt.axes().minorticks_on()
	plt.axes().tick_params(axis='both', which='major', direction='in', length= 8, width=2)
	plt.axes().tick_params(axis='both', which='minor', direction='in', length= 4, width=1.5)
	plt.axes().xaxis.set_major_locator(MultipleLocator(100))
...
def main(): 
	...
	parser.add_argument('-AB2', '--input_AB1', type=str, help='Path to ancestral bases count around barriers', default ="/home/saraoukkal/Documents/Stage_M1/Chimp_Step4_results/AB_count_bar.txt")
	parser.add_argument('-Gen2', '--input_Gen1', type=str, help='Path to bases count around barriers', default ="/home/saraoukkal/Documents/Stage_M1/Chimp_Step4_results/AB_count_bar.txt")
